...
const callGemini = async (prompt: string): Promise<string> => {
  try {
    if (!API_KEY) return "ERROR: API key is missing. Check VITE_GEMINI_KEY in Vercel.";
    ...
    if (data?.error) return "Gemini Error: " + data.error.message + " (code: " + data.error.code + ")";
    return data.candidates?.[0]?.content?.parts?.[0]?.text || "No response from Gemini. Raw: " + JSON.stringify(data);
  } catch (err: any) {
